  ...
  <ion-page>
    ...
    <ion-content :fullscreen="true">
      <ion-header collapse="condense">
        <ion-toolbar>
          <ion-title size="large">Physical needs</ion-title>
        </ion-toolbar>
      </ion-header>
      ...
      <div id="container">
        <ion-card id="physical-needs-understanding">
          ...
          <ion-card-content>
            <p>Physical needs or physical difficulties ( PD) is an umbrella term covering a range of conditions and impairments; each individual will have unique needs arising from their disability. The impact may range from mild to severe and affect their ability to take part in activities in the same way as others.</p>
          </ion-card-content>
        </ion-card>
                ...
                <div class="ion-padding" slot="content">
                  <ion-list v-if="languageUseLines.length">
                    <ion-item v-for="(line, index) in languageUseLines" :key="'use-' + index">
                      ...
                      <ion-label>
                        <h4 style="margin: 0; white-space: pre-wrap;">{{ ensureSentenceEnding(line) }}</h4>
                      </ion-label>
                    </ion-item>
                  </ion-list>
...
const loadQuizState = () => {
  ...
  quizCompletionCount.value = ProgressService.getQuizCompletionCount('physical-sensory-needs');
...
const saveCurrentAnswer = () => {
  ...
  } else if (q.type === 'multi-true-false' || q.type === 'fill-in-blank') {
...
const calculateQuizScore = (): number => {
  ...
  questions.value.forEach((question: any, index: number) => {
    ...
    if (question.type === 'multi-true-false') {
      ...
      const ua = userAnswer as Record<string, string> | undefined;
      if (ua && question.sentences?.every((sentence: FillSentence) =>
        getSentenceBlanks(sentence).every((blank) => ua[blank.id] === blank.correctAnswer)
      )) {
        correctAnswers += 1;
      }
